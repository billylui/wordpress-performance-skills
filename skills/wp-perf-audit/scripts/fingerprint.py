...
An operator-evidence document has the shape
{"cache_layers":[{"layer":"server","operator_confirmed":{...}}]}.
"""
...
# Operator evidence is restricted to the contract's three authenticated access tiers.
MIN_OPERATOR_EVIDENCE_TIER = 1
# Tier 3 is the highest access tier defined by the shared contract.
MAX_OPERATOR_EVIDENCE_TIER = 3
...
# Cache layers have fixed cardinality and order in stack-profile schema 1.1.
CACHE_LAYER_ORDER = ("edge", "server", "page-plugin", "object")
# Each operator-confirmed value must come from the named layer's closed contract vocabulary.
CACHE_VALUES_BY_LAYER = {
    "edge": (
        "cloudflare", "cloudflare-apo", "quic-cloud", "bunny", "keycdn", "fastly",
        "akamai", "stackpath", "aws-cloudfront", "none", "other", "unknown",
    ),
    "server": (
        "litespeed", "nginx-fastcgi", "varnish", "batcache", "other", "none", "unknown",
    ),
    "page-plugin": (
        "wp-rocket", "litespeed-cache", "w3-total-cache", "wp-super-cache",
        "wp-fastest-cache", "sg-optimizer", "breeze", "surge", "cache-enabler",
        "none", "unknown",
    ),
    "object": (
        "redis", "memcached", "apcu", "object-cache-pro", "none", "unknown",
    ),
}
# Input entries mirror cache-layer output entries and admit no unattributed fields.
OPERATOR_EVIDENCE_ENTRY_FIELDS = ("layer", "operator_confirmed")
# Confirmations carry evidence metadata only; public confidence remains a separate signal.
OPERATOR_CONFIRMATION_FIELDS = ("value", "tier", "evidence", "confirmed_by")
...
# This cache-specific header proves a server cache, but its name is not
# vendor-namespaced and therefore cannot identify the product exclusively.
NON_NAMESPACED_CACHE_HEADER = "x-gateway-cache-status"
NON_NAMESPACED_CACHE_CONFIDENCE = "medium"
# The stack-profile vocabulary uses `other` when a proven server cache product is unidentified.
UNIDENTIFIED_SERVER_CACHE_VALUE = "other"
...
def detect_cache_layers(
    ...
    if (
        server_signal["value"] == "unknown"
        and NON_NAMESPACED_CACHE_HEADER in headers
    ):
        server_signal = make_signal(
            UNIDENTIFIED_SERVER_CACHE_VALUE,
            NON_NAMESPACED_CACHE_CONFIDENCE,
            [
                "header: {}: {}".format(
                    NON_NAMESPACED_CACHE_HEADER,
                    headers[NON_NAMESPACED_CACHE_HEADER],
                )
            ],
        )
...
def unique_json_object(pairs: List[Tuple[str, object]]) -> Dict[str, object]:
    """Build a JSON object while refusing duplicate fields hidden by ordinary decoding."""

    result: Dict[str, object] = {}
    for name, value in pairs:
        if name in result:
            raise ValueError("duplicate JSON field " + repr(name))
        result[name] = value
    return result


def require_exact_fields(value: Dict[str, object], expected: Sequence[str], field: str) -> None:
    """Refuse missing or unknown fields with a path an operator can act on."""

    expected_set = set(expected)
    actual_set = set(value)
    missing = sorted(expected_set - actual_set)
    if missing:
        raise ValueError(field + " is missing field " + repr(missing[0]))
    unexpected = sorted(actual_set - expected_set)
    if unexpected:
        raise ValueError(field + " has unknown field " + repr(unexpected[0]))


def load_operator_evidence(path: str) -> List[Tuple[str, Dict[str, object]]]:
    """Read and validate higher-tier cache evidence without assigning a public verdict."""

    try:
        with open(path, "r", encoding="utf-8") as source:
            document = json.load(source, object_pairs_hook=unique_json_object)
    except OSError as exc:
        raise ValueError("operator-evidence file could not be read: " + str(exc)) from exc
    except (UnicodeError, json.JSONDecodeError, ValueError) as exc:
        raise ValueError("operator-evidence file is malformed JSON: " + str(exc)) from exc

    if not isinstance(document, dict):
        raise ValueError("operator-evidence document must be an object")
    require_exact_fields(document, ("cache_layers",), "operator-evidence document")
    entries = document["cache_layers"]
    if not isinstance(entries, list):
        raise ValueError("cache_layers must be an array")

    confirmations: List[Tuple[str, Dict[str, object]]] = []
    seen_layers = set()
    for index, entry in enumerate(entries):
        entry_field = "cache_layers[" + str(index) + "]"
        if not isinstance(entry, dict):
            raise ValueError(entry_field + " must be an object")
        require_exact_fields(entry, OPERATOR_EVIDENCE_ENTRY_FIELDS, entry_field)

        layer = entry["layer"]
        layer_field = entry_field + ".layer"
        if not isinstance(layer, str) or layer not in CACHE_LAYER_ORDER:
            raise ValueError(
                layer_field + " must be one of " + ", ".join(CACHE_LAYER_ORDER)
            )
        if layer in seen_layers:
            raise ValueError(layer_field + " duplicates layer " + repr(layer))
        seen_layers.add(layer)

        confirmation = entry["operator_confirmed"]
        confirmation_field = entry_field + ".operator_confirmed"
        if not isinstance(confirmation, dict):
            raise ValueError(confirmation_field + " must be an object")
        ...
        value_field = confirmation_field + ".value"
        if not isinstance(value, str) or value not in CACHE_VALUES_BY_LAYER[layer]:
            raise ValueError(
                value_field
                + " must be one of "
                + ", ".join(CACHE_VALUES_BY_LAYER[layer])
            )

        tier = confirmation["tier"]
        tier_field = confirmation_field + ".tier"
        if (
            type(tier) is not int
            or tier < MIN_OPERATOR_EVIDENCE_TIER
            or tier > MAX_OPERATOR_EVIDENCE_TIER
        ):
            raise ValueError(
                tier_field
                + " must be an integer from "
                + str(MIN_OPERATOR_EVIDENCE_TIER)
                + " to "
                + str(MAX_OPERATOR_EVIDENCE_TIER)
            )

        evidence = confirmation["evidence"]
        evidence_field = confirmation_field + ".evidence"
        if (
            not isinstance(evidence, list)
            or not evidence
            or not all(isinstance(item, str) and item.strip() for item in evidence)
        ):
            raise ValueError(evidence_field + " must be a non-empty array of non-empty strings")

        confirmed_by = confirmation["confirmed_by"]
        confirmed_by_field = confirmation_field + ".confirmed_by"
        if not isinstance(confirmed_by, str) or not confirmed_by.strip():
            raise ValueError(confirmed_by_field + " must be a non-empty string")
    ...
    return sorted(confirmations, key=lambda item: CACHE_LAYER_ORDER.index(item[0]))


def apply_operator_evidence(
    cache_layers: Sequence[Dict[str, object]],
    confirmations: Sequence[Tuple[str, Dict[str, object]]],
) -> List[Dict[str, object]]:
    """Attach evidence only where the public probe left the cache layer unknown."""

    result = [dict(entry) for entry in cache_layers]
    layers_by_name = {str(entry["layer"]): entry for entry in result}
    for layer, confirmation in confirmations:
        public = layers_by_name[layer]
        if public["value"] != "unknown":
            raise ValueError(
                "operator evidence for cache layer "
                + repr(layer)
                + " conflicts with public value "
                + repr(public["value"])
                + "; public response evidence takes precedence"
            )
        public["operator_confirmed"] = {
            "value": confirmation["value"],
            "tier": confirmation["tier"],
...
def human_report(document: Dict[str, object]) -> str:
    ...
    cache_layers = document["cache_layers"]
    assert isinstance(cache_layers, list)
    for cache_layer in cache_layers:
        assert isinstance(cache_layer, dict)
        label = "Cache " + str(cache_layer["layer"]).title()
        lines.append(
            label
            + ": "
            + str(cache_layer["value"])
            + " ("
            + str(cache_layer["confidence"])
            + ")"
        )
        confirmation = cache_layer.get("operator_confirmed")
        if isinstance(confirmation, dict):
            lines.append(
                label
                + " Operator-Confirmed: "
                + str(confirmation["value"])
                + " (tier "
                + str(confirmation["tier"])
                + "; confirmed by "
            ...
            for item in evidence:
                lines.append(label + " Operator Evidence: " + str(item))
    return "\n".join(lines) + "\n"
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--operator-evidence",
        metavar="PATH",
        help="attach validated higher-tier cache-layer evidence from PATH",
    )
    parser.add_argument(
        "--pages",
...
def run(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.operator_evidence:
        try:
            confirmations = load_operator_evidence(args.operator_evidence)
        except ValueError as exc:
    ...
    if confirmations:
        ...
        assert isinstance(cache_layers, list)
        try:
            document["cache_layers"] = apply_operator_evidence(
                cache_layers,
                confirmations,
            )
        except ValueError as exc:
