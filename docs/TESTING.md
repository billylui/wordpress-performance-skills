...
| WP-ESC-18 | 2026-08-14 | A capability gap told the operator to supply a provider that could not help: with no `--target`, LCP/INP/CLS gaps kept `kind: "provider"` and an `unlock` naming Lighthouse/MCP/PSI, none of which measures anything without a URL. The first fix prepended the missing target to the human `blocked_by` string and left `kind` and `unlock` untouched | **A fix applied to the human string and not to the machine-readable field.** The defect was about what a consumer is *told*, and consumers read the structure; changing the sentence closed the ticket and left the behaviour. It was caught only because an independent review re-read the same finding and recognised its own earlier finding coming back — which is what "sibling" means and why a sibling stops the loop instead of opening a round. The same session produced a second instance of the shape: a release-gate row passed by checking the schema changes I had in mind rather than every schema change in the diff | Asking, of any finding about output, **which representation is authoritative** — then writing the assertion against that one. A test that reads the prose cannot fail a prose-only fix, so the pair must read the structure. Mutation-test it by restoring the shallow fix: the structural cases must fail while the prose case still passes | `tools/adversarial_gate_tests.py` §"a gap names the prerequisite that is actionable NOW" — 7 cases, 4 of them controls, including the with-a-target control that stops "re-key everything to access" passing vacuously; `docs/handoffs/capability-gap-followups.md` |
